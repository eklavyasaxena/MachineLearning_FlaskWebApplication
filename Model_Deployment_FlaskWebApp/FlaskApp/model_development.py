...
def uploadToS3(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, upload_filePath, local_filePath = '', destinationPath = ''):
    if local_filePath == '':
        local_filePath = upload_filePath
    ...
    k.set_contents_from_filename(local_filePath, cb = percent_cb, num_cb = 10)
    print('Uploaded')

def data_transformation():
    response = S3.get_object(Bucket=BUCKET_NAME, Key=DATA_DIRECTORY+'/'+'churn_train.csv')
    training_dataset = response['Body'].read()
    
    df = pd.read_csv(io.BytesIO(training_dataset))
